...
public OnPluginStart()
{
	...
	cvarHUD = CreateConVar("sm_mostrador", "1", "Muestra si el hay capitan o no en la ronda actual", _, true, 1.0);
...
public Action:DOMenu(client,args)
{
	...
	PrintToChat(client, "\x04[Internacional ZP] \x05%t" ,"Tus creditos", g_creditos[client]);
...
CreateMenuClient(clientId) 
{
	if(menus[clientId] == INVALID_HANDLE)
	{
		...
		SetMenuTitle(menus[clientId], "Internacional Zombie Plague");
	...
	if ( action == MenuAction_Select ) 
	{
		...
		if (g_creditos[client] >= Items[precio])
		{
			if (IsPlayerAlive(client))
			{
				...
				{
					PrintToChat(client, "\x04[Internacional ZP] \x05%t","Este premio no esta disponible para tu equipo");
				}
			}
			else
			{
		...
		{
			PrintToChat(client, "\x04[Internacional ZP] \x05%t","Necesitas creditos", g_creditos[client],Items[precio]);
		}
		//DID(client);
...
public Action:InicioRonda(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	new Float:intervalo = GetConVarFloat(cvarHUD);
	for (new idx = 1; idx <= MaxClients ; idx++)
	{
		timerhud = CreateTimer(intervalo, HUD, idx, TIMER_REPEAT);
	}
...
public Action:HUD(Handle:timer, any:client)
{
	...
	SetHudTextParams(0.03, 0.9, 2.0, 255, 0, 0, 255);
	if(IsClientInGame(client))
	{
		ShowSyncHudText(client, HudText, "Tus Creditos: %i", g_creditos[client]);
	}
}