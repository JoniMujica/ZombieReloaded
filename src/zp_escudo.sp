...
public ZP_OnAwardBought( client, const String:awardbought[])
{
	if(StrEqual(awardbought, AWARDNAME))
	{
		...
		PrintToChat(client, " \x04[Internacional ZP] \x05%t", "You bought an human shield");
		inmunidad[client] = 5;
	}
}
...
public Action:ZR_OnClientInfect(&client, &attacker, &bool:motherInfect, &bool:respawnOverride, &bool:respawn)
{
	...
	if(inmunidad[client] > 0)
	{
		...
		EmitSoundToAllAny("franug/zombie_plague/resistencia_armadura.wav", SOUND_FROM_WORLD, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL, -1, pos);
		PrintToChat(client, "\x04[Internacional ZP] \x05Te quedam %i resistencia de armadura", inmunidad[client]);
