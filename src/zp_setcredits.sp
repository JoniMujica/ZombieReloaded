...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_zpcredits", FijarCreditos, ADMFLAG_CUSTOM2);
}
...
public Action:FijarCreditos(client, args)
{
	...
	{
		ReplyToCommand(client, "%t","[SM] Utiliza: sm_zpcredits <#userid|nombre> [cantidad]");
	...
	{ 
		...
		{ 
			...
			ZP_SetCredits(iClient, amount);
			decl String:nombre[64];
			GetClientName(client, nombre, 64);
			PrintToChat(client, "\x04[Internacional ZP] \x05%t","Puesto creditos", amount, nombre);
		} 
	} 
	//}  
